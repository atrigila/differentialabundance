...
if (! is.null(opt\$round_digits)){
  comp.results <- format_numeric_columns(comp.results, opt\$round_digits)
}
...
if (! is.null(opt\$round_digits)){
  normalised_matrix <- format_numeric_columns(normalised_matrix, opt\$round_digits)
}
...
for (vs_method_name in strsplit(opt\$vs_method, ',')){
  ...
  if (! is.null(opt\$round_digits)){
    vs_mat <- format_numeric_columns(vs_mat, opt\$round_digits)
  }
